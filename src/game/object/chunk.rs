...
impl Chunk {
    ...
    pub(crate) fn copy_buffer(&self, renderer: &mut Renderer, buffer: &StorageBuffer, index:u32) {
        self.grid.buffer.copy_from_storage_buffer(renderer, buffer, index as u64 *std::mem::size_of::<ModelUniform>() as u64);
    }
}